...
class sx126x:
    ...
    def change_mode(self,mode):
        if mode == "normal":   #For normal operation
            GPIO.output(self.M0, GPIO.LOW)
            GPIO.output(self.M1, GPIO.LOW)
        if mode == "config": #changing settings
            GPIO.output(self.M0,GPIO.LOW)
            GPIO.output(self.M1,GPIO.HIGH)
        if mode == "wake on radio": # Wake on LAN similar
            GPIO.output(self.M0,GPIO.HIGH)
            GPIO.output(self.M1,GPIO.LOW)
        if mode == "sleep": # Putting Lora to deep sleep (Lower Power Consumption)
            GPIO.output(self.M0,GPIO.HIGH)
            GPIO.output(self.M1,GPIO.HIGH)

    def lora_sleep(self):
        self.change_mode("sleep")
    ...
    def get_settings(self):
        ...
        if self.get_reg[0] == 0xC1 and self.get_reg[2] == 0x09:
            ...
            print(f"Frequencey : {fre_temp} Hz")
            print(f"Node address is {addr_temp}")
            print(f"Air speed is { self.lora_air_speed_dic.get(None,air_speed_temp)} bps")
            print(f"Power is {self.lora_power_dic.get(None,power_temp)} dBm" )
            GPIO.output(self.M1,GPIO.LOW)
